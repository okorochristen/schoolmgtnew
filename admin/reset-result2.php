<?php
  ...
  if (isset($_SESSION['alogin']) && !empty($_SESSION['alogin'])) {
    if (!empty($_GET['class'] && !empty($_GET['class2'])) && !empty($_GET['term']) && !empty($_GET['session'])) {
      ...
      $q = $db->prepare("delete from result2 where class = ? && sclass = ? && term = ? && session = ?");
      $q->bind_param('ssss',$class,$class2,$term,$session);
      $q->execute();
      $q->close();

      if($term == 3){
        $q = $db->prepare("delete from promotional_scores2 where session = ? and class = ? and class2 = ?");
        $q->bind_param('sss',$session,$class,$class2);
        $q->execute();
        $q->close();

        $q = $db->prepare("select regno, average from promotional_result2 where session = ? and class = ? and class2 = ?");
        $q->bind_param('sss',$session,$class,$class2);
        $q->execute();
        $q->store_result();
        ...
        $q = $db->prepare("delete from promotional_result2 where session = ? and class = ? and class2 = ?");
        $q->bind_param('sss',$session,$class,$class2);
        $q->execute();
        $q->close();
      }
      ...
      header("Location:view-primary-result.php?class=$class&class2=$class2&term=$term&session=$session");
      exit;
    }
    else {
