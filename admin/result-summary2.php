...
<?php if ( (isset($_SESSION['alogin'])) && (strlen($_SESSION['alogin']) >  0) ): ?>
  ...
  <?php if ( (isset($_REQUEST['class'])) && (isset($_REQUEST['session'])) && (isset($_REQUEST['class2'])) ):
    ...
    $q = $db->prepare('select distinct regno, average, position, status from promotional_result2 where class = ? and session = ? and class2 = ?');
              ...
              <tbody>
               ...
               ?>
                <tr>
                  ...
                  <td>
                  <?php if ($status == "0"): ?>
                    <button type="button" name="button" class="btn btn-outline-danger">Not promoted</button>
                  <?php elseif ($status == "1"): ?>
                    <button type="button" name="button" class="btn btn-outline-success">promoted</button>
                  <?php endif; ?>
                  </td>
                  <td><a class="btn btn-outline-info" href="student-result-summary2.php?regno=<?php echo $regno; ?>&session=<?php echo $session; ?>&class=<?php echo $class; ?>&class2=<?php echo $class2; ?>" target="_blank">View</a></td>
                </tr>
              <?php endwhile; ?>
            </tbody>
          </table>
