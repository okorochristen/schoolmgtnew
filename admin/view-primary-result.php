...
<?php if ( (isset($_SESSION['alogin'])) && (!empty($_SESSION['alogin'])) ): ?>
  <?php if (!empty($_REQUEST['session']) && !empty($_REQUEST['class2']) && !empty($_REQUEST['term']) && !empty($_REQUEST['class'])):
    ...
    $q = $db->prepare("select distinct subject from scores2 where class = ? && class2 = ? && term = ? && session = ?");
      ...
      <div class="container" style="margin:50px; padding-left:250px">
        ...
        <div class="col-md-12">
          <?php if ($c > 0):
            ...
            $q1 = $db->prepare("select count(id) from result2 where class = ? && sclass = ? && term = ? && session = ?");
            ...
            <table>
              ...
              <tbody>
                  ...
                  <?php while ($q->fetch()):
                    ...
                    $q0 = $db->prepare("select count(id) from scores2 where class = ? && class2 = ? && term = ? && session = ? && subject = ?");
                    ...
                    $q0 = $db->prepare("select count(id) from scores2 where class = ? && class2 = ? && term = ? && session = ? && subject = ? && total >= ? && total < ?");
                    $start_score = 0;
                    $end_score = 40;
                  ...
                  <tr>
                    ...
                    <td> <a href="view-subject-scores2.php?<?php echo 'class='.$class.'&class2='.$class2.'&term='.$term.'&session='.$session.'&subject='.$subjectid; ?>">View</a> </td>
                  </tr>
                <?php endwhile; ?>
                <tr>
                  <?php if ($n < 1): ?>
                    <td> <a href="compute-result2.php?<?php echo 'class='.$class.'&class2='.$class2.'&term='.$term.'&session='.$session; ?>" class="btn btn-primary pull-right">Compute</a> </td>
                  <?php else: ?>
                    <td> <a href="reset-result2.php? <?php echo 'class='.$class.'&class2='.$class2.'&term='.$term.'&session='.$session; ?>" class="btn btn-primary pull-right">Reset Result</a> </td>
                  <?php endif; ?>
                </tr>
              </tbody>
            </table>
          <?php else: ?>
            <h4>No scores  have been entered for this class this term. <a href="dashboard.php?<?php echo 'class='.$class.'&class2='.$class2.'&term='.$term.'&session='.$session; ?>" class="btn btn-default">Back</a> </h4>
          <?php endif; ?>
