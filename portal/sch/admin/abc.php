...
<?php if (isset($_SESSION['alogin'])): ?>
  <?php if (isset($_GET['id']) && isset($_GET['class']) && isset($_GET['session'])):
    ...
    $q = $db->prepare("select fname from students where regno = ?");
    $q->bind_param('s',$id);
    $q->execute();
    $q->bind_result($name);
    $q->fetch();
    ...
    $q = $db->prepare("select count(id) from accepted_students where class = 'a' and school = 'secondary' and current_class = ? and current_session = ?");
    ...
    $q = $db->prepare("select count(id) from accepted_students where class = 'b' and school = 'secondary' and current_class = ? and current_session = ?");
    $q->bind_param('ss',$classid, $session);
    $q->execute();
    $q->bind_result($nb);
    $q->fetch();
    ...
    $q = $db->prepare("select count(id) from accepted_students where class = 'c' and school = 'secondary' and current_class = ? and current_session = ?");
    $q->bind_param('ss',$classid, $session);
    $q->execute();
    $q->bind_result($nc);
    $q->fetch();
    ...
    $q = $db->prepare("select count(id) from accepted_students where class = 'd' and school = 'secondary' and current_class = ? and current_session = ?");
    $q->bind_param('ss',$classid, $session);
    $q->execute();
    $q->bind_result($nd);
    $q->fetch();
    ...
    $q = $db->prepare("select count(id) from accepted_students where class = 'e' and school = 'secondary' and current_class = ? and current_session = ?");
    $q->bind_param('ss',$classid, $session);
    $q->execute();
    $q->bind_result($ne);
    $q->fetch();
          ...
          <div class="container">
            <div class="panel panel-default" style="border: 1px solid black; padding:10px; margin-top:50px; border-radius:4px;">
              ...
              <div class="panel-body">
                <form class="" action="accept-student.php?stud_id=<?php echo $id; ?>" method="post" id="clf">
