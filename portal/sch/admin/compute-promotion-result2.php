<?php
  ...
  if ( (isset($_SESSION['alogin'])) && (strlen($_SESSION['alogin']) >  0) ) {
    if ( (isset($_REQUEST['session'])) && (isset($_REQUEST['class'])) && (isset($_REQUEST['class2'])) ) {
      ...
      $q = $db->prepare('select distinct regno from promotional_scores2 where class = ? and session = ? and class2 = ?');
      ...
      if ($n > 0){
        ...
        while ($q->fetch()) {
          ...
          $q1 = $db->prepare('select total from promotional_scores2 where regno = ? and session = ? and class = ? and class2 = ?');
        ...
        foreach ($average as $key => $value) {
          ...
          ++$p;
          $q0 = $db->prepare("insert into promotional_result2 (regno, session, class, class2, total, average, position, class_average, status) values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
          $q0->bind_param('sssssssss', $key, $session, $class, $class2, $sgtotal["$key"], $value, $p, $class_average, $promote);
          $q0->execute();
          $q0->close();
        }
        $db->close();
        header("Location:result-summary2.php?&session=$session&class=$class&class2=$class2");
        exit;
      }
      else {
        $q->close();
