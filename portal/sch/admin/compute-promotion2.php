<?php
  ...
  if ( (isset($_SESSION['alogin'])) && (strlen($_SESSION['alogin']) >  0) ) {
      if ( (isset($_REQUEST['session'])) && (isset($_REQUEST['class'])) && (isset($_REQUEST['class2'])) ) {
        ...
        $q = $db->prepare("select distinct subject from scores2 where session = ? and class = ? and class2 = ?");
        ...
        $q = $db->prepare('select distinct regno from accepted_students where current_class = ? and current_session = ? and school = ? and class = ?');
        ...
        while ($q->fetch()) {
          ...
          for($i = 0; $i < $x; $i++){
            ...
            $scq = $db->prepare("select total from scores2 where subject = ? and regno = ? and class = ? and session = ? and class2 = ?");
            echo "$db->error";
        ...
        foreach ($data as $sub_key => $sub_value) {
          ...
          foreach ($sub_value as $reg_key => $score_value) {
            ...
            $q = $db->prepare("insert into promotional_scores2 (regno, session, class, class2, subject, total, grade, class_average,position) values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $db->close();
        header("Location:compute-promotion-result2.php?session=$session&class=$class&class2=$class2");
        exit;
      }
    }
?>
