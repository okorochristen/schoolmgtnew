<?php
  ...
  if ( (isset($_SESSION['login'])) && (strlen($_SESSION['login']) >  0) ) {
      if ( (isset($_GET['session'])) && (isset($_GET['class'])) ) {
        ...
        $q = $db->prepare("select distinct subject from scores2 where session = ? and class = ?");
        $q->bind_param('ss',$session,$class);
        $q->execute();
        $q->store_result();
        ...
        $q = $db->prepare('select regno from accepted_students where current_class = ? and current_session = ? and school = ?');
        $q->bind_param('sss',$class,$session,$school);
        $q->execute();
        $q->store_result();
        $q->bind_result($regno);
        $y = 0;
        while ($q->fetch()) {
          ...
          for($i = 0; $i < $x; $i++){
            ...
            $scq = $db->prepare("select total from scores2 where subject = ? and regno = ? and class = ? and session = ?");
            $scq->bind_param('ssss', $sub_array[$i],$regno,$class,$session);
            $scq->execute();
            $scq->store_result();
        ...
        foreach ($data as $sub_key => $sub_value) {
          ...
          foreach ($sub_value as $reg_key => $score_value) {
            ...
            $q = $db->prepare("insert into promotional_scores2 (regno, session, class, subject, total, grade, class_average,position) values (?, ?, ?, ?, ?, ?, ?, ?)");
            $q->bind_param('ssssssss',$reg_key, $session, $class, $sub_key, $score_value, $grade, $classAverageInSubject, $p);
            $q->execute();
            $q->close();
            ++$p;
          }
        ...
        $db->close();
        header("Location:compute-promotion-result2.php?session=$session&class=$class");
        exit;
      }
    }
?>
