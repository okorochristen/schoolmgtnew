<?php
  ...
  if ( (isset($_SESSION['alogin'])) && (strlen($_SESSION['alogin']) >  0) ) {
    if ( (isset($_GET['session'])) && (isset($_GET['term'])) && (isset($_GET['class'])) && (isset($_GET['class2'])) ) {
      ...
      $q = $db->prepare("select count(id) from secondary_behaviour where class = ? && class2 = ? && term = ? and session = ?");
      ...
      if ($ca > 0) {
        ...
        if ($n > 0){
          ...
          header("Location:view-secondary-result.php?term=$term&session=$session&class=$class&class2=$class2");
          exit;
        }
        else {
          $q->free_result();
        ...
        header("Location:view-secondary-result.php?session=$session&term=$term&class=$class&class2=$class2");
        exit;
      }
    }
    else {
