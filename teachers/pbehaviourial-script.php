<?php
  ...
  if ( (isset($_SESSION['login'])) && (!empty($_SESSION['login']) ) ){
      if (isset($_REQUEST['class']) && isset($_REQUEST['class2']) && isset($_REQUEST['session']) && isset($_REQUEST['term']) && isset($_REQUEST['bhv_btn'])) {
        ...
        if ($q->num_rows > 0) {
          $q0 = $db->prepare("insert into primary_behaviour (regno, session, term, class, class2, punctuality, attendance, assignment, school_act, neatness, honesty, self_control, relationship, games, lab) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
          while ($q->fetch()) {
          ...
          header("Location:enter-behaviourial-ratingsp.php?session=$session&term=$term&class=$class&class2=$class2");
          exit;
        }
        else {
