  ...
  if ( (isset($_SESSION['login'])) && (!empty($_SESSION['login']) ) ){
    if (isset($_POST['ss_btn'])) {
      if ((!isset($_POST['class'])) || (!isset($_POST['class2'])) || (!isset($_POST['session'])) || (!isset($_POST['term'])) || (!isset($_POST['subject']))) {
        ...
        $q->store_result();
        $ns = $q->num_rows;
        $q1 = $db->prepare('select total from scores2 where regno = ? and session = ? and term = ? and class = ? and subject = ? and class2 = ?');
        while ($q->fetch()) {
          $regs[$c] = $regnos;
          echo "$regs[$c]<br>";
          $c++;
        ...
        $q1 = $db->prepare('update scores2 set class_average = ? where session = ? and term = ? and class = ? and subject = ? and class2 = ?');
        $q1->bind_param('ssssss', $caverage, $session, $term, $class, $subject, $class2);
        $q1->execute();
        $q1->close();
        $q1 = $db->prepare('select count(total) from scores2 where regno = ? and session = ? and term = ? and class = ? and subject = ? and class2 = ?');
        $q0 = $db->prepare('insert into scores2 (regno, session, term, class, class2, subject, as1, as2, ts1, ts2, exam, total, grade, class_average, staffid) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        $al = count($regs);
        for ($i = 0; $i < $al; $i++) {
          $regnos = $regs[$i];
          $q1->bind_param('ssssss', $regnos, $session, $term, $class, $subject, $class2);
        ...
        $db->close();
        $_SESSION['allow'] = 1;
        header("Location:enter-scores2.php?class=$class&class2=$class2&subject=$subject&term=$term");
      }
    }
    else {
      header('Location:enter-scores2.php');
      exit;
    }
  }
  else {
