...
<?php if ( (isset($_SESSION['login'])) && (!empty($_SESSION['login'])) ):
    ...
    elseif ( (empty($_POST['session'])) && (!isset($_GET['session'])) ) {
      ...
      $q2->free_result();
      $q2->close();
      $q0 = $db->prepare('select regno from scores where class = ? && class2 = ? && session = ? and term = ?');
...
<div class="container" style="padding-top: 60px">
          ...
          <tbody>
            ...
            <?php while ($q0->fetch()): ?>
              <?php
                ...
                $q = $db->prepare('select distinct as1, as2, ts1, ts2, exam, total from scores where session = ? and term = ? and class = ? and subject = ? and regno = ? and term = ?');
  ...
  <div class="col-md-6 pull-left" style="margin-bottom: 30px">
      <a href="view-secondary-result.php?<?php echo 'class='.$class.'&class2='.$class2.'&term='.$term.'&session='.$session; ?>" class="btn btn-outline-info">Go Back</a>
  </div>
</div>
